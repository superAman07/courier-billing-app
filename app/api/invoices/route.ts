    ...
    const createdInvoice = await runWithRetries(async () => {
      return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
        ...
        if (customerType === 'CASH') {
             // --- 1. NEW LOGIC FOR CASH (0001, 0002...) ---
             
             // Find max existing number for BookingMaster_CASH type invoices
             // We check for simple 4-digit numbers to find the next active one
             const existingCashInvoices = await tx.invoice.findMany({
                where: { type: 'BookingMaster_CASH' },
                select: { invoiceNo: true }
             });
             // Extract max number from "0001", "0025", etc.
             const maxNum = existingCashInvoices.reduce((max, inv) => {
                 if (/^\d+$/.test(inv.invoiceNo)) {
             ...
             const invoiceNo = String(nextNum).padStart(4, '0');
