...
const normalizeSector = (input: string): string | null => {
    ...
    // Normalize logic for fuzzy match
...
export async function POST(req: NextRequest) {
    try {
        ...
        // Ensure empty cells default to empty string so headers align
        ...
        // 1. Bulk Fetch Customers for Mapping
        // Creates a clean list of unique customer codes from the file to fetch IDs in one go
        ...
        for (let i = 0; i < rows.length; i += BATCH_SIZE) {
            ...
            await Promise.all(batch.map(async (row, batchIndex) => {
                const rowNum = i + batchIndex + 2; // Excel row number (1-based + header)
                ...
                if (!customerId) {
                    errors.push({ row: rowNum, customer: code, sector: rawSector, reason: `Customer Code not found` });
                    return;
                }
                ...
                try {
                    ...
                    errors.push({ row: rowNum, customer: code, sector: rawSector, reason: "DB Error" });
                }
            })); // End Batch Promise
        } // End Loop
