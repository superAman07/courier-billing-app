        ...
        <div className="bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8">
            <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-xl overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                    ...
                    <select name="cityId" value={form.cityId} onChange={handleChange} className="p-2 border cursor-pointer rounded text-gray-500 border-gray-300" required>
                        <option value="">-- Select City --</option>
                        {filteredCities.map(c => <option key={c.id} value={c.id}>{c.name.toUpperCase()}</option>)}
                    </select>
                ...
                <div className="px-6 pb-2 flex items-center space-x-2">
                    ...
                    <select value={cityFilter} onChange={e => setCityFilter(e.target.value)} className="p-2 border rounded text-gray-500 border-gray-300">
                        ...
                        {filterCitiesForDropdown.map(c => <option key={c.id} value={c.id}>{c.name.toUpperCase()}</option>)}
                    </select>
                        ...
                        <tbody>
                            {filtered.map((pin, idx) => (
                                <tr key={pin.pincode}>
                                    ...
                                    <td className="px-3 py-2 text-gray-600 uppercase">{pin.state?.name.toUpperCase() || ''}</td>
                                    <td className="px-3 py-2 text-gray-600 uppercase">{pin.city?.name.toUpperCase() || ''}</td>
                                    <td className="px-3 py-2 text-gray-600 uppercase">{pin.city?.code.toUpperCase() || ''}</td>
